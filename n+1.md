...
The API for this fairly strait forward.  The annotation takes the Class of the child type, the property name(s) of
the parent key (onProperties) and the child key (toProperties). *Yes it is possible to join on multiple keys)*. 
...
**If anyone can figure out a nice way for me to detect this. I would fail fast instead of going into a loop.** 

This is testable from the Unit Tests if you want to try it (Just run AllLocalTests which only uses local DBs).

Currently, I'm working on a benchmarking project using the StackOverflow2010 10 gigabyte DB.
[Here](https://www.brentozar.com/archive/2015/10/how-to-download-the-stack-overflow-database-via-bittorrent/) 
I'll put this up on github soon. 
