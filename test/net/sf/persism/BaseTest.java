...
public abstract class BaseTest extends TestCase {
    ...
    public void testQueryWithSpecificColumnsWhereCaseDoesNotMatch() throws SQLException {
        ...
        List<Customer> list = session.query(Customer.class, sql);
    ...
    public void testQueryResultRecord() throws Exception {
        ...
        results = session.query(CustomerOrderRec.class, sql);
        log.info(results);
        assertEquals("size should be 4", 4, results.size());
        ...
        var fail = session.query(CustomerOrderRec.class, sql);
        log.warn(fail);
    ...
    public void testQueryResultRecordNegative() throws Exception {
        ...
        try {
            var fail1 = session.query(CustomerOrderGarbage.class, sql);
        ...
        try {
            var fail2 = session.query(CustomerOrderGarbage.class, sql);
...
//            }
    ...
    public void testInvoiceRec() {
        ...
        InvoiceRec invoice = new InvoiceRec("MOO", 10.5f, 10, 0, new BigDecimal("10.23"), true);

        Result<InvoiceRec> result = session.insert(invoice);
        ...
        List<InvoiceRec> invoices = session.query(InvoiceRec.class, "select * from Invoices where CUSTOMER_ID=?", "MOO");
        log.info(invoices);
        assertEquals("invoices s/b 1", 1, invoices.size());
        ...
        assertTrue("paid s/b true", invoice.paid());

        NumberFormat nf = NumberFormat.getInstance();

        assertEquals("totals/b 105.00", nf.format(105.0f), invoice.total().toString());
    }
    ...
    public void testRecord2() {
        ...
        log.info("testRecord2 ALL? \n" + session.query(RecordTest2.class, "select * from RecordTest2"));
        ...
        boolean fail = false;
        try {
            rt23 = session.fetch(RecordTest2.class, "select PRiCE, QtY, DESCrIPTION, iD FROM RecordTest2 where ID = ?", 2);
...
//        }
    ...
    static String sql(String sql) {
        return sql;
    }

    static Object[] params(Object... arr) {
        return arr;
    }
}
